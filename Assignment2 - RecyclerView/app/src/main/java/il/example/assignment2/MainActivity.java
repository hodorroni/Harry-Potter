...
public class MainActivity extends AppCompatActivity {
    ...
    public ArrayList<DataModel> listSearch(ArrayList<DataModel> list,String searchedCharacter){
       ...
       for(DataModel obj : list){
           searchedCharacter = searchedCharacter.toLowerCase();
           if(obj.getName().toLowerCase().contains(searchedCharacter) || obj.getDescription().toLowerCase().contains(searchedCharacter)){
               newList.add(obj);
           }
       }
    ...
    public void createDataSetTable(){
        ...
        adapter = new CharactersAdapter(dataset, new CharactersAdapter.ItemListener() {
            ...
            public void onItemClicked(int index) {
                ...
                final Snackbar[] snack = new Snackbar[1];
                 snack[0] = Snackbar.make(rootView,dataset.get(index).getExtraText(),Snackbar.LENGTH_LONG).setAction("Dismiss", new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        snack[0].dismiss();
                    }
                }).setDuration(15000);
                View snackBarView = snack[0].getView();
